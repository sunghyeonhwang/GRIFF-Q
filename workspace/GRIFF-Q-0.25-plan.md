...
> **최종 업데이트: 2026-02-28 (7차 — 전 Phase 완료, v0.25 마무리)**
...
- [x] 모든 서버 액션 버튼에 로딩 상태 표시
- [x] 다크모드에서 차트/배지 색상 정상 표시
...
### Phase D — 2026-02-28 완료

**커밋:** `546100c` (`ui: Phase D 폼 UX 개선 — 인라인 검증 + 이탈 경고 + 키보드 단축키`)

> Phase D 상세 로그는 DEVLOG.md 참조

- D1: 인라인 유효성 검사 (Zod 스키마)
- D2: Unsaved Changes 경고 (beforeunload + 네비게이션 가드)
- D3: 자동 저장 상태 표시
- D4: 키보드 단축키 (Ctrl+S 저장)

### Phase E+F — 2026-02-28 완료

**커밋:** `4251725` (`ui: Phase E+F 시각 디자인 & 마이크로 인터랙션 구현`)
**변경:** 16 files, +220 / -61
...
| 항목 | 내용 |
|---|---|
| 유틸리티 클래스 (6개) | `text-page-title`, `text-page-description`, `text-section-title`, `text-card-title`, `text-stat-number`, `text-stat-unit` |
| 정의 위치 | `globals.css` `@layer utilities` |
| 적용 | `page-header.tsx` (전 페이지 일괄), `dashboard/page.tsx` (요약 카드, 섹션 제목) |

#### E3 아이콘 사용 일관성

| 항목 | 내용 |
|---|---|
| 검증 결과 | 대부분 맥락에 맞게 사용 (인라인 size-4, 소형 size-3.5, 빈 상태 size-12) |
| 변경 | 불필요 — 이미 일관적 |
...
| 항목 | 내용 |
|---|---|
| 카드 테두리 | `card.tsx`에 `dark:border-white/15` 추가 — 다크모드 카드 구분 강화 |
| 사이드바 경계 | `--sidebar-border` 10% → 12% 미세 조정 |
| 차트/배지 | 기존 CSS 변수 대응으로 이미 정상 — 변경 불필요 |

#### F1 버튼 로딩 스피너 통일

| 항목 | 내용 |
|---|---|
| 신규 파일 | `src/components/ui/loading-button.tsx` |
| 기능 | `loading` prop → Loader2 animate-spin + 텍스트 변경 + disabled |
| 적용 (9개) | meeting-form, avatar-form, payment-form, estimate-form, retrospective-form, postmortem-form, project-create-dialog, login-form, meetings/import |
| 패턴 교체 | `{loading ? "저장 중..." : "저장"}` → `<LoadingButton loading={loading} loadingText="저장 중...">저장</LoadingButton>` |

#### F2 리스트 항목 애니메이션

| 항목 | 내용 |
|---|---|
| keyframes | `fade-slide-in` (opacity 0→1 + translateY 8px→0, 0.3s ease-out) |
| 유틸리티 | `.animate-fade-slide-in` (`globals.css`) |
| 적용 | `table.tsx` TableRow에 클래스 추가 |

#### F3 호버 프리뷰 Tooltip

| 항목 | 내용 |
|---|---|
| 마감 임박 테이블 | 마감일 호버 → "D-2", "D-day" 등 잔여일 표시 |
| 최근 회의록 | 회의록 항목 호버 → 전체 제목 + 상세 날짜 (요일 포함) 표시 |
| 컴포넌트 | Radix `Tooltip` (기존 shadcn 컴포넌트 활용) |

#### F4 숫자 카운트업 애니메이션

| 항목 | 내용 |
|---|---|
| 신규 파일 | `src/components/ui/count-up.tsx` |
| 기능 | `requestAnimationFrame` + `easeOutQuart`, 0→target 800ms 카운트업 |
| 적용 | 대시보드 요약 카드 4개 (마감 임박, 입금 대기, 최근 회의, 프로젝트) |

#### 테스트

- `next build` 에러 0건 ✅
- Chrome DevTools MCP 브라우저 직접 테스트 (라이트/다크 × 모바일/데스크톱 4조합) ✅
- Tooltip 호버 프리뷰 동작 확인 (마감일 D-2 표시) ✅
- 숫자 카운트업 애니메이션 정상 ✅
- 테이블 행 진입 애니메이션 정상 ✅
- 다크모드 카드 테두리 가시성 개선 확인 ✅
...
**기간:** 2026-02-28 (1일)
**총 Phase:** 6개 (A~F), 24개 작업 전체 완료
**핵심 성과:**
- 공통 인프라 정비 (Breadcrumb, PageHeader, Toast, Skeleton, EmptyState, 에러 바운더리)
- 대시보드 리디자인 (Bento 그리드, 차트 3종, 활동 타임라인, 퀵 액션)
- 테이블/목록 UX (서버사이드 페이지네이션 7개, 컬럼 정렬, 카드 뷰 토글)
- 폼 UX (인라인 검증, 이탈 경고, 자동 저장, 키보드 단축키)
- 시각 디자인 (브랜드 컬러 시스템, 타이포그래피 통일, 다크모드 조정)
- 마이크로 인터랙션 (LoadingButton, 행 애니메이션, Tooltip, 카운트업)
