...
- [ ] 5. 견적서 공동 작업(E): Supabase Realtime+Presence 기반 실시간 동시 편집. PDF/Google Sheets/Excel 퍼블리시. 작년 견적 비교 + 하이라이트
- [ ] 6. 입금/결제 요청(B): 이름/금액/은행/계좌번호/입금자명/마감일/비고 입력 폼 + 은행명 중심 리스트 뷰
- [ ] 7. 클라이언트 반응 예측(F): 클라이언트별 페르소나/톤 프리셋 저장, Gemini API 연동 채팅 UI
- [ ] 8. 변경 이력: 주요 테이블에 audit log 트리거, 작업자별 수정 로그, 관리자 롤백
